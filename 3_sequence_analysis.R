...
names(df_seq)[-1] <- paste(str_sub(names(df_seq)[-1], -2, -1), 
                             str_sub(names(df_seq)[-1], 4, -3),
                             sep = ".")
...
seq_all <- df_seq %>%
  ...
  seqdef(states = state_names, cpal = state_cols,
         ...
         weights = df_weight$Survey_Weight_W8)
...
ward_clust <- hclust(as.dist(seq_dists), method="ward",
                   members = df_weight$Survey_Weight_W8[any_neet])
...
clustering <- clust_qual$clustering
